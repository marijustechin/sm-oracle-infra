...
Prepared [docs/application-deployment-contract.md](docs/application-deployment-contract.md) (design only; no deployment, no application source access). It records the interface between this repository and the future application repository in three clearly separated categories: **(A) accepted infrastructure decisions** (GHCR; GitHub Actions + Buildx; prebuilt `linux/arm64` images; no builds on the Oracle host; deployment by immutable OCI digest; accepted edge/app/db networking; Nginx TLS termination; infrastructure-owned PostgreSQL runtime; file-based secret delivery; no direct public frontend/API/PostgreSQL publication), **(B) proposed application contract defaults** (ports 3000/3001, `/health/ready`, UID 10001, `/api` ownership — recommended, explicitly not final facts until the application accepts them), and **(C) application-owned open fields** (image digests, migration command, environment/file-secret names, writable paths, persistent-storage and egress requirements, graceful-shutdown confirmation). It adds an explicit ownership/boundary section: the application repository owns source, Dockerfiles, migrations, health implementation/tests and image builds; this repository owns Compose/runtime config, digest pins, Nginx routing, PostgreSQL runtime, secret provisioning and deployment/rollback; application CI builds/publishes images but does not receive production SSH credentials or auto-deploy in the initial model.

The image delivery section keeps GHCR + GitHub Actions/Buildx + ARM64-only + digest pinning as the recommended initial model, and a proposed (not implemented) Compose skeleton is included. Ports, health paths, migration commands, UIDs and secret names remain open fields rather than final facts; section C lists the nine blockers. Updated README (file table) and TODO (Section 7 pointer). No Compose or application files were created, no live access, no commit or push.

### Section 4 Docker host reconciled and closed — Human accepted

Reconciled Section 4, which the earlier acceptance (commit `228ed18`) had left open for one deferred verification item: "verify container forwarding/NAT and Docker-published ports with OCI rules for the actual deployment; keep direct application/database ports private; recheck after any authorized firewall reload/reboot before claiming persistence." Later Section 6 evidence satisfies each part: Docker port publication on TCP 80/443 was exercised and externally verified (bootstrap DNAT/publication counters and the HTTPS `nat DOCKER` DNAT rules to `172.18.0.2`), no unintended public Docker ports appeared (TCP 111/3000/3001/5432 closed throughout, only the reviewed proxy publishing 80/443), and a controlled reboot confirmed Docker/containerd enabled+active, `restart: unless-stopped` containers auto-returning with 80/443 restored, and saved firewall/`sshd`/`InstanceServices` unchanged. Marked Section 4 closed / Human accepted; the remaining application/database port-privacy check belongs to the future application deployment in Section 7. No live inspection or configuration change was performed to close this documentation. No push.
