...
**Section 4 reconciliation — 2026-09-08:** the deferred container forwarding/published-port verification recorded above was subsequently completed during Section 6. The HTTP bootstrap and HTTPS activation exercised and externally verified Docker port publication on TCP 80/443 and the DNAT/FORWARD path ([HTTP bootstrap evidence](#http-bootstrap-verified--2026-09-08--ready-for-review)), no unintended public Docker ports appeared, and the [reboot persistence test](#reboot-persistence-verified--section-6-closed--2026-09-08--human-accepted) confirmed Docker/containerd enabled+active with `restart: unless-stopped` containers auto-returning and 80/443 publications restored while saved firewall/`sshd`/`InstanceServices` remained unchanged. Section 4 is therefore closed; the remaining application/database port-privacy check belongs to the future application deployment (Section 7).
